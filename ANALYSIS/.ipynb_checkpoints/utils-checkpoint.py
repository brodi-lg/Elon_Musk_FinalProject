...
# Add this function for toxicity detection
def detect_toxicity(df, text_column='Comment'):
    """
    Detect toxicity in comments using the Detoxify model.
    ...
    Returns:
        pd.DataFrame: The DataFrame with an additional 'Toxicity' column.
    """
    # Initialize Detoxify
    detoxify_model = Detoxify('original')

    # Apply toxicity detection
    df['Toxicity'] = df[text_column].apply(lambda x: detoxify_model.predict(x)['toxicity'])

    # Plot toxicity distribution
    plt.figure(figsize=(8, 5))
    sns.histplot(df['Toxicity'], bins=20, kde=True, color='red')
